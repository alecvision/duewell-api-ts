...
const PLAID_PRODUCTS = (process.env.PLAID_PRODUCTS || 'transactions').split(',') as Products[];
const PLAID_COUNTRY_CODES = (process.env.PLAID_COUNTRY_CODES || 'US').split(',') as CountryCode[];
const PLAID_REDIRECT_URI = (process.env.PLAID_REDIRECT_URI || '') as string | undefined;
...
router.post('/create_link_token', function (_request, response, next) {
    Promise.resolve().then(async function () {
        const configs = {
            ...
            client_name: 'Plaid Quickstart', //TODO change client name
            products: PLAID_PRODUCTS,
